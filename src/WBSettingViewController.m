...
static NSInteger const delayTag = 12345;
static NSInteger const sportCountTag = 54321;
...
- (void)addBasicSettingSection {
    ...
    [sectionInfo addCell:[self sportCountCell]];
...
- (MMTableViewCellInfo *)sportCountCell {
    NSInteger sportCount = [WBRedEnvelopConfig sharedConfig].sportCount;
    NSString *sportCountString = [NSString stringWithFormat:@"%ld 步", (long)sportCount];
    MMTableViewCellInfo *cellInfo = [objc_getClass("MMTableViewCellInfo") normalCellForSel:@selector(settingSportCount) target:self title:@"微信运动步数" rightValue: sportCountString accessoryType:1];
    return cellInfo;
}
...
- (void)settingSportCount {
    UIAlertView *alert = [UIAlertView new];
    alert.title = @"微信运动(步数)";
    alert.tag = sportCountTag;
    ...
    [alert textFieldAtIndex:0].placeholder = @"微信运动";
    [alert textFieldAtIndex:0].keyboardType = UIKeyboardTypeNumberPad;
    [alert show];
}

- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (alertView.tag == delayTag && buttonIndex == 1) {
    ...
    if (alertView.tag == sportCountTag && buttonIndex == 1) {
        NSString *sportCountString = [alertView textFieldAtIndex:0].text;
        NSInteger sportCount = [sportCountString integerValue];
        
        [WBRedEnvelopConfig sharedConfig].sportCount = sportCount;
